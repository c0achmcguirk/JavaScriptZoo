...
interface AJBase {
  ...
  getByTagName(tagName: string) : any;
  getElementsByTagNameAndAttr(tagName : string, attrName : string, attrValue : string, domNode ?: HTMLElement) : any;
  getUrlParam(url : string, paramName : string) : any;
  postFetch(url : any, type :string, onerror ?: any) : any;
}

var AmberjackBase : AJBase = {
  ...
  postFetch: function(url, type, onerror?) {
    if (type === 'script') {
      ...
      theStyle.href = url;
      if (onerror) { theStyle.onerror = onerror; }
    }
...
interface AJControlBase {
  open(tplHtml:string) : any;
  close() : any;
}

var AmberjackControl : AJControlBase = {
...
interface AJ {
  ...
  textPrev: string;
  textNext: string;
  onCloseClickStay     : boolean;
  doCoverBody          : boolean;
  bodyCoverCloseOnClick: boolean;
  urlPassTourParams    : boolean;
  pageId: any;
  pages: any;
  pageCount : number;
  pageCurrent ?: string;
...
var Amberjack : AJ = {
  ...
  getWindowInnerHeight: function() {
    ...
    else if (document.documentElement && document.documentElement.scrollHeight > (<HTMLElement>document.documentElement).offsetHeight){ // Explorer 6 strict mode
